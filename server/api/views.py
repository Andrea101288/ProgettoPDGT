...
class Earthquakes(generic.View):
    def get(self, request, country):
        ...
        today = datetime.now()
        # Get last year date
        last_year = today - timedelta(days=7)

        # Url encode
        today = urllib.parse.quote_plus(today.isoformat())
        last_year = urllib.parse.quote_plus(last_year.isoformat())

        url = "http://webservices.ingv.it/fdsnws/event/1/query?starttime={0}&endtime={1}&minmag=2&maxmag=10&format=atom".format(last_year, today)
